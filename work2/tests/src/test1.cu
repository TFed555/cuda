...
TYPED_TEST(MatrixTest, MultiMatrix) {
  Matrix<float> a(2, 2);
  Matrix<float> b(2, 2);
  Matrix<float> res(2, 2);
  MatrixXf expected = MatrixXf::Constant(2, 2, 8.0f);
...
TYPED_TEST(MatrixTest, MultiScalarMatrix) {
  Matrix<float> a(2, 2);
  ...
  MatrixXf expected = MatrixXf::Constant(2, 2, 6.0f);
  
  Matrix<float> res = a.view() * scalar;
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 2; ++j) {
      EXPECT_FLOAT_EQ(hostRes[i*2+j], expected(i, j));
    }
  }
}